...
5. NEXT: SS-2.6 (SS-23) RAG retrieval (ADR-0001 retrieval node — vector similarity over chunk, return cited chunks scoped to team/sprint) -> SS-2.7 (SS-22) baseline detector that turns the SS-2.3 eval GREEN (4/4 + 4/4). Stage-1: 5 of 7 done (SS-19/20/21/18/24). Open items still pending real wiring (not blocking the build): provision Supabase Postgres+pgvector (only CI uses the ephemeral DB so far) and finalise the in-region 1024-dim embedding model (D1) to replace HashingEmbedder.
