...
- NEXT (slice 6): per-tenant RLS policies — needs a least-privilege NON-OWNER DB role + a per-request tenant GUC (the `postgres` owner BYPASSES RLS today, so policies alone do nothing); team_id is populated and the retriever is already tenant-scoped, so this is the enforcement layer.
