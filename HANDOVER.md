...
## Where we are
Stage 7 (UX Polish + Connectors, Epic SS-5), SECOND SLICE built on branch `stage7-jira-connector` (NOT yet merged; awaiting David's review). The first real connector: a read-only Jira reader that turns live tickets into the existing `Artifact` shape, proven end to end by a script (Goal A, "prove the pipe"). Not the cross-tool watermelon on live data (that is Goal B, a later slice).
...
- REMAINING (David, clean-network day): create the empty sandbox Jira project and give its key; confirm a Composio API key; then run `seed_demo_board.py <KEY>` and `run_connector_demo.py --project <KEY>` live. Spec: docs/superpowers/specs/2026-06-24-jira-connector-design.md; plan: docs/superpowers/plans/2026-06-24-jira-connector.md.
- LIVE-RUN VERIFY (flagged by review, isolated in `connector.py`): on the first live pull, confirm the Composio response envelope shape (`fetch_issues` assumes `raw["data"]["issues"]`) and that each issue's `description` and `comments` come back as plain strings, NOT Atlassian Document Format (ADF, nested JSON). If they are ADF, add a flattener in `_to_clean` (the single mapping home). No offline test depends on these; they cannot be known without the live account.
