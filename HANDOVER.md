...
- NEXT (slice 7): least-privilege NON-OWNER DB role — create `app_rw` (non-superuser), grant least privilege, operator switches the live `DATABASE_URL` to it; then the slice-6 policy actually ENFORCES for the app (and `TO <app_role>` scoping restores 0002's hard deny for the PostgREST anon/authenticated roles). The policy + GUC wiring are already in place; this is the activation step.
