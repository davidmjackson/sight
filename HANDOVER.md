...
- NEXT (slice 5): real Supabase Auth + login CSRF + per-tenant RLS policies (team_id now populated; retriever already tenant-scoped).
- LATER: load the remaining delivery-domain rows (sprint/metric/burndown/dependency/signal) -> then the portfolio verdict/report could also run off DB rows (the rest of Approach B).
