...
Last updated: 2026-06-27 (Claude Code session 22: FIVE slices + a Langfuse cost-fix shipped. LATEST — REAL SUPABASE AUTH WIRING DONE + MERGED @ 216b5d7: `SupabaseAuthenticator` now verifies passwords against Supabase Auth (GoTrue password grant, anon key only) behind a fail-safe gate (`make_authenticator()`: offline `SeedAuthenticator` by default; Supabase only when `SPRINTSIGHT_AUTH=supabase` + `SUPABASE_URL` + `SUPABASE_ANON_KEY`). Session model unchanged (verify creds -> return User; app keeps its signed cookie). SECURITY: role read ONLY from admin-set `app_metadata`, never user-editable `user_metadata` (no self-escalation); fails closed. No new dep. Eval-first `tests/web/auth/test_supabase_auth.py` (17, offline, httpx faked); conftest forces `SPRINTSIGHT_AUTH=seed` for hermeticity; review Ready to merge (0 Critical/Important, 2 minors applied); 313 passed + 4 skipped, ruff clean. Operator step to go live: create users + set app_metadata.role + `SPRINTSIGHT_AUTH=supabase` (docs/auth/supabase-auth.md). Also this session: LANGFUSE COST FIX @ e67b699 (tracing now opt-in via `SPRINTSIGHT_TRACE=on` to protect the free-tier quota; no payment needed). EARLIER: SLICE 7 — LEAST-PRIVILEGE app_rw ROLE DONE + MERGED + CI-PROVEN @ b56b049: `db/migrations/0004` creates `app_rw` (NOLOGIN/NOSUPERUSER/NOBYPASSRLS, no password in git) + least-privilege grants; the CI `db` job now runs ingest + retrieval AS `app_rw` and asserts it is subject to RLS (0 rows without a tenant GUC, 5 teams with it, not superuser/bypassrls) — the enforcement proof slice 6 couldn't give. NO app code change (connecting as app_rw is a DATABASE_URL change). Policy NOT scoped TO app_rw (PUBLIC policy already denies anon; TO-scoping would lock the owner out). Review Ready to merge; CI green. OPERATOR STEP to go live: `alter role app_rw with login password` + repoint live DATABASE_URL via the SESSION pooler (docs/db/app-role.md). Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-app-role*. EARLIER THIS SESSION: SLICE 6 — PER-TENANT RLS POLICIES DONE + MERGED + CI-PROVEN @ 00de42e: `db/migrations/0003` adds a `tenant_isolation` policy (FORCE RLS + USING/WITH CHECK on `tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid`) on all 8 tenant tables; the app sets `app.tenant_id` per connection (`PostgresStore`/`PostgresRetriever`). CI `db` job proves cross-tenant no-leak via `db/checks/rls_isolation.sql`; offline `tests/test_tenant_guc.py` pins the GUC wiring. KEY FINDING (caught by CI): SUPERUSERS + the table OWNER bypass RLS and FORCE only subjects the owner not a superuser, so the policy is correct + proven (eval runs as a non-superuser SET ROLE probe) but the APP is protected only once it connects as a least-privilege non-superuser role (NEXT = slice 7). Also `set_config(...,NULL,...)` stores '' so the policy uses nullif to fail closed. Review Ready to merge; CI fully green. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-tenant-rls-policies*. EARLIER THIS SESSION: SLICE 5 — LOGIN CSRF DONE + MERGED: `POST /login` now requires a per-session synchronizer token (`issue_csrf`/`valid_csrf`, stdlib `secrets`, in the signed session cookie), checked BEFORE authentication so a forged cross-site POST never probes credentials; fails closed (missing/wrong/foreign-session/non-ASCII token -> clean 400); closes the login-CSRF gap deferred at SS-34. Eval-first `tests/web/auth/test_csrf.py` (7); the shared login() test helper now fetches the token so the whole suite exercises the guard; independent review Ready to merge (0 Critical/Important, 1 minor applied); 290 passed + 4 skipped, ruff clean; eval gates unchanged. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-login-csrf*. NEXT slice 6 = per-tenant RLS policies (needs a least-privilege non-owner DB role + a per-request tenant GUC; the postgres owner bypasses RLS today). EARLIER THIS SESSION: REAL-WIRING SLICE 4 — WEB READS EVIDENCE FROM THE DB DONE + MERGED. The `/team` drill-in now shows a "live DB" panel of that team's cited chunks read from Supabase via the team-scoped `PostgresRetriever` (slice-3 payoff), behind a new off-by-default fail-safe gate `_db_enabled()` (`SPRINTSIGHT_WEB_DB=on` + `DATABASE_URL`); any DB error yields an empty panel, never a 500. DELIBERATE SCOPE: only the evidence panel reads the DB — the watermelon verdict + report stay corpus-computed because the DB stores the generic shape and lacks the sprint/semantic-id/functional-type the detector needs (verdict-on-DB would need a schema change + re-ingest). Schema UNCHANGED; eval-first `tests/web/test_db_read.py` (14, offline fake retriever); independent review Ready to merge (0 Critical/Important, 2 minors applied); 283 passed + 4 skipped, ruff clean; deterministic eval gates unchanged. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-web-db-read*. PRIOR session 21: TWO real-wiring slices shipped. SLICE 3 — POPULATE TEAM_ID DONE + MERGED @ 597efb5: ingestion now creates the 5 `team` rows and links every artifact via `artifact.team_id` (`Store.upsert_team` on both stores, resolved up front in `ingest_corpus`; `counts()` gains `team`), and `PostgresRetriever` gained an optional `team` filter (tenant-scoped, left-joins `team`, returns the key) — the scoping its docstring said was "awaiting team_id population". Eval-first `tests/test_team_id.py` (in-memory: 5 teams, every artifact linked, correct mapping, idempotent) + the CI `db` job now asserts 5 teams + zero null team_id + a team-scoped no-leak smoke check in `retrieve_smoke.py`. Schema UNCHANGED (columns already existed). Backfill note: the live Supabase still has NULL team_id; it backfills on the next FULL re-ingest, which the slice-2 real-embedder switch forces (or force it per docs/embedder/real-embedder.md). Independent review = Ready to merge, 0 Critical/Important, 2 minors applied (tenant-scope the global search too; runbook edge case). 269 passed + 4 skipped, ruff clean. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-populate-team-id*. EARLIER THIS SESSION: SLICE 2 — REAL EMBEDDER (decision D1) DONE + MERGED to main @ 0954a9a + LIVE-VERIFIED. Swapped the fake `HashingEmbedder` (matches only identical words) for a real, self-hostable, in-region 1024-dim model (`thenlper/gte-large`) via new `LocalEmbedder` + `make_embedder()` factory in `sprintsight/ingest/embedding.py`, selected by env `SPRINTSIGHT_EMBEDDER=local` (offline `HashingEmbedder` stays the default, so CI + the LangGraph builder + the web/connector demos are unchanged). `sentence-transformers` lives in a new lazy `[embed]` pyproject extra; the schema is UNTOUCHED (`chunk.embedding vector(1024)`). The ingest + retrieve scripts both call the factory so ingest and query always use the same embedder. KEY FIX from the independent review (the blocker): the ingest dedup hash now folds in the embedder identity (`embedder_signature` -> `pipeline._content_hash`), so switching the embedder against an already-populated store (the slice-1 Supabase, loaded with the fake) RE-EMBEDS every artifact instead of silently skipping it on an unchanged body and leaving stale, incomparable vectors. Eval-first: new `tests/test_embedder_real.py` (semantic-vs-lexical paraphrase recall; CI baseline pinned <=0.25, the real-model leg gated/skipped) + a re-embed test in `tests/test_ingest.py`. LIVE-VERIFIED: real model clears paraphrase recall >=0.75 vs the stand-in's 0.25. Independent whole-branch review = blocker found, fixed, re-verified CLOSED, Ready to merge. Full suite 265 passed + 4 skipped, ruff clean; deterministic eval gates unchanged. Runbook: docs/embedder/real-embedder.md; spec/plan: docs/superpowers/{specs,plans}/2026-06-27-real-embedder*. NOTE: the real model is wired + eval-proven, but re-embedding the live Supabase end-to-end is a separate operator step (run the runbook; first re-ingest re-embeds all 37 even with the same stand-in because the hash FORMAT changed — a harmless one-time refresh). EARLIER session 20: SECURITY HARDENING — migration `db/migrations/0002_enable_rls.sql` enables Row Level Security (no policies) on all 8 public tables, clearing the Supabase Security Advisor "RLS not enabled" findings; safe because the `postgres` owner role bypasses RLS and Sprintsight never uses the PostgREST public API (anon key), verified live via `retrieve_smoke.py`; the live instance was fixed by pasting the 8 `enable row level security` statements into the Supabase SQL editor (migrate.py is not re-runnable); per-tenant `tenant_id` policies deferred to the real-auth slice; merged to main @ 8da61f5, tests 261 passed + 3 skipped unchanged. EARLIER session 18: STAGE 7 COMPOSIO SDK PORT slice complete and MERGED to main — the live Jira connector (`sprintsight/connect/connector.py:fetch_issues`) was ported from the removed `ComposioToolSet().execute_action(...)` API to the current `composio==0.16` `Composio().tools.execute(...)` client, identifying the connected account via a new env var `COMPOSIO_CONNECTED_ACCOUNT_ID`; the `/crosstool` live gate now also requires that id so the offline/live badge stays honest; `composio` pinned `>=0.16,<0.17`. Built subagent-driven, 4 tasks each reviewed clean + opus whole-branch review = Ready to merge, 0 Critical/Important (2 Minors applied). Full suite 245 passed + 3 skipped, ruff clean; eval gates 4/4 + 4/4 + cross-tool 7/7 unchanged. LIVE-VERIFIED end to end (2026-06-26, @ a22eb0c): a real human-run read of SSSB returned 6 tickets (top cited SSSB-1). Three composio-0.16 quirks were fixed live: plain-dict response (read dict-first), required `user_id`/`COMPOSIO_USER_ID` alongside `connected_account_id` (gate now needs SEVEN vars), and `dangerously_skip_version_check=True`. docs/connectors/live-run.md documents all three env vars + the editable-install requirement. UPDATE (session 19, 2026-06-26): the full `/crosstool` WEB live demo is now DONE + LIVE-VERIFIED. A logged-in web request read the real SSSB board + the real sprintsight-sandbox repo live behind the seven-var gate; badge showed `live as of 2026-06-26T07:50:02Z` and flagged the two cross-tool watermelons (Atlas/SSSB-1 no-ref, Boreas/SSSB-2 PR#1 open-unmerged), agreeing exactly with the CLI read. No new code; verification only. Prior session 17: STAGE 7 cross-tool LIVE-WIRE slice complete and MERGED to main — the `/crosstool` page can now read the LIVE Jira board + GitHub repo behind a fail-safe gate (`SPRINTSIGHT_CROSSTOOL_LIVE`=on + GITHUB_TOKEN + Composio key + repo/project), offline-by-default and deterministic in CI; a live failure falls back to the offline replay and says so ("offline (live read failed)"), never a 500 and never fake-live. See "Where we are" below. Prior: STAGE 6 first slice (Portfolio + Watermelon UI, Epic SS-6) on branch `stage6-watermelon-ui`. Prior: STAGE 4 (Observability + Evals, Epic SS-7, Story SS-30) complete on branch `stage4-observability-llm-judge`. LLM-as-judge readability scorer added (`sprintsight/evals/judge.py`): four prose dimensions (clarity, audience_fit, coherence, actionability), injected grader (fake in CI, real Anthropic on the key-gated path), advisory pass bar (every dim >= 3, mean >= 4, non-gating). Calibration meta-eval added (`sprintsight/evals/calibration.py`): grades the judge against hand-labelled good/bad anchor reports before it becomes a gate. Per-node graph tracing added to `sprintsight/graph/builder.py` via optional Tracer (no-op default, CI stays offline; one `graph:run` span wraps the run, each node emits a `node:<name>` span). ADR-0003 records the tracing design. Opt-in `--judge` flag on `scripts/run_report_eval.py` runs the readability pass (advisory, never changes exit code). `scripts/run_calibration.py` runs the live calibration. Both new modules green in CI with fakes; live paths key-gated. Deterministic watermelon + report evals unchanged, still the CI gate. Prior: STAGE 3 DONE (SS-29, branch stage3-langgraph-graph); STAGE 2 DONE (arc 2, 374bf6c); STAGE 0 + STAGE 1 CLOSED. Repo on github.com/davidmjackson/sight; CI green. System of record: Jira (statuses) + the docs below (specs/decisions). FOLLOW-UP THIS SESSION (2026-06-19): writer-readability arc on branch `writer-readability-arc` (see the dedicated section below).
...
- Real identity provider (Supabase Auth), and why a role must come from admin-set metadata | login can now check passwords against Supabase Auth (the managed identity service) instead of our checked-in demo file, behind the same off-by-default flag pattern; the security idea for a non-engineer = a logged-in user can edit some of their own profile data ("user_metadata"), so if we read their ROLE from there they could promote themselves to admin; we therefore read the role only from "app_metadata", which only an administrator can set, and default to the lowest-privilege role otherwise; also "fail closed" = if the identity service is unreachable or the password is wrong, login is denied, never allowed | sprintsight/web/auth/users.py SupabaseAuthenticator/_role_from + make_authenticator gate + docs/auth/supabase-auth.md + supabase-auth slice | flagged 2026-06-27
- Why observability should be opt-in (a quota/cost lesson) | Langfuse (our tracing/eval-inspection tool) has a free monthly quota; the tracer used to switch on whenever the Langfuse keys were present, so once the app started loading the `.env` (persistent-Supabase slice) every local eval/graph run quietly shipped trace events and we blew the 50k/month free limit; fix = make tracing opt-in (off unless `SPRINTSIGHT_TRACE=on` AND the keys are set), same flag-plus-credentials pattern as the LLM/DB/crosstool/embedder gates; the idea for a non-engineer = third-party tools often meter usage, and a "fires whenever a key exists" integration can silently run up a bill/quota, so default it OFF | sprintsight/evals/tracing.py `_enabled`/`get_tracer` + SPRINTSIGHT_TRACE flag + langfuse-trace-gate | flagged 2026-06-27
- Least privilege: the app logs in as a restricted database user, not the owner | following on from the RLS lesson, the database rule only protects a "normal" user, so we created `app_rw` (a login that can only read/write the app's tables, nothing more, and cannot bypass the rule) and the app now connects as it instead of the all-powerful owner; the idea for a non-engineer = "least privilege" (give each piece of software the smallest set of permissions it needs, so a bug or a stolen password can do less damage) and that this is the step that finally switches tenant isolation on for the app | db/migrations/0004_app_role.sql + docs/db/app-role.md + the CI db job proving the app works as app_rw under enforced RLS + app-role slice | flagged 2026-06-27
- Per-tenant Row Level Security, and why "enable RLS" is not "enforce RLS" | RLS = the database itself filters which rows a query can see by a rule, so a forgotten WHERE clause cannot leak another tenant's data; the non-obvious lesson a non-engineer needs = the database only applies that rule to a "normal" login, NOT to a SUPERUSER or the table's owner (they bypass it), so adding the rule is not the same as it taking effect; we proved the rule is correct (a normal role is properly isolated) but the app gains the protection only once it connects as a least-privilege normal role (the next step) | db/migrations/0003_tenant_rls_policies.sql + db/checks/rls_isolation.sql + PostgresStore/PostgresRetriever set_config app.tenant_id + tenant-rls slice | flagged 2026-06-27
- CSRF protection on the login form (a synchronizer token) | "CSRF" = a malicious page in another tab silently submitting our login form for you (logging you into the attacker's account); the fix = the sign-in page plants a secret one-time token tied to your browser session, and the server refuses any login POST without the matching token (an attacker's page cannot read it); the new idea for a non-engineer = why a login itself needs CSRF protection (login-CSRF), and that the check runs BEFORE the password so a forged request cannot even probe credentials | sprintsight/web/auth/session.py issue_csrf/valid_csrf + app.py POST /login + login-csrf slice | flagged 2026-06-27
- A web screen reading from the live database for the first time (and why only the evidence panel) | the team drill-in now shows a "live DB" panel of that team's cited chunks read from Supabase via the team-scoped retriever, behind the same off-by-default fail-safe gate we keep reusing; the new idea for a non-engineer = why the watermelon verdict still can't read from the DB (the DB stores a generic shape that drops the sprint number, the semantic id, and the functional type the detector needs, so verdict-on-DB would need a schema change + a re-load) | sprintsight/web/service.py db_knowledge_for/_db_enabled + team.html live-DB panel + web-db-read slice | flagged 2026-06-27
- Linking the data to a team (a foreign key) so the DB can scope by team | the database had a `team` table and a `team_id` slot on every artifact, but both were empty, so the DB itself could not answer "just Atlas's items"; we now create the team rows and fill in each artifact's team_id at load time, which lets the production search filter to one team; the new idea for a non-engineer = a "foreign key" (one row pointing at another by its id) and why a column existing in the schema is not the same as it being filled in | sprintsight/ingest/store.py upsert_team + pipeline team_ids + sprintsight/retrieval/postgres.py team filter + populate-team-id slice | flagged 2026-06-27
...
- DONE (slice 3, session 21): populate `team` rows + `artifact.team_id` at ingest; `PostgresRetriever` team scoping (tenant-scoped). (Loading sprint/metric/burndown/dependency/signal rows is still a separate later slice.) Live backfill rides the slice-2 re-embed re-ingest (or force it; see runbook).
- DONE (slice 4, session 22): the web team drill-in reads cited evidence FROM the DB (Approach B, scoped). New fail-safe gate `_db_enabled()` (`SPRINTSIGHT_WEB_DB=on` + `DATABASE_URL`, off by default) + `db_knowledge_for(team)` serve a "live DB" evidence panel via the already team-scoped `PostgresRetriever` (slice-3 payoff); fail-safe (any DB error -> empty panel, never a 500). DELIBERATE SCOPE (David's call 2026-06-27): only the evidence panel reads the DB; the watermelon verdict + report stay corpus-computed because the DB stores the generic shape and lacks the sprint/semantic-id/functional-type the detector is hard-wired to (verdict-on-DB would need a schema change + re-ingest). Schema UNCHANGED. Eval-first `tests/web/test_db_read.py` (14, injected fake retriever, fully offline). Independent review = Ready to merge, 0 Critical/Important, 2 of 4 minors applied (clamp display score; connect-time failure test). 283 passed + 4 skipped, ruff clean. Operator step to see it live: set `SPRINTSIGHT_WEB_DB=on` + `DATABASE_URL` + `SPRINTSIGHT_EMBEDDER=local` (match ingest) + `SPRINTSIGHT_ENV=dev`, log in, open `/team/atlas`. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-web-db-read*.
- DONE (slice 5, session 22): login CSRF protection. `POST /login` now requires a per-session synchronizer token (`issue_csrf`/`valid_csrf` in `sprintsight/web/auth/session.py`, stdlib `secrets`, stored in the signed session cookie), checked BEFORE authentication so a forged cross-site POST cannot probe credentials; fails closed (missing/wrong/foreign-session/non-ASCII token -> clean 400). Closes the login-CSRF gap deferred at SS-34 (session 10). Eval-first `tests/web/auth/test_csrf.py` (7); shared `login()` test helper now fetches the token so the whole suite exercises the guard. Independent review = Ready to merge, 0 Critical/Important, 1 minor applied (non-ASCII token -> clean 400). 290 passed + 4 skipped, ruff clean. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-login-csrf*.
- DONE (slice 6, session 22): per-tenant RLS POLICIES, DB-enforced + CI-proven. `db/migrations/0003_tenant_rls_policies.sql` adds a `tenant_isolation` policy (FORCE RLS + `USING`/`WITH CHECK` on `tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid`) on all 8 tenant tables; the app announces its tenant on connect via `set_config('app.tenant_id', <uuid>, false)` in `PostgresStore`/`PostgresRetriever`. CI `db` job proves cross-tenant no-leak via `db/checks/rls_isolation.sql` (A can't see B, B can't see A, unset-GUC fail-closed). Offline `tests/test_tenant_guc.py` (3, faked psycopg) pins the GUC wiring. KEY FINDING (caught by CI, not the review): RLS is bypassed by SUPERUSERS and the table OWNER, and FORCE only subjects the owner, NOT a superuser — so the policy is correct + proven (the eval runs as a non-superuser `SET ROLE` probe) but the APP gains real protection only once it connects as a least-privilege NON-superuser role. Also fixed: `set_config(...,NULL,...)` stores '' not unset, so the policy uses `nullif(...,'')` to fail closed. Independent review = Ready to merge (0 Critical, reviewed pre-superuser-fix). CI fully green @ 00bc73e. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-tenant-rls-policies*.
- DONE (slice 7, session 22): least-privilege `app_rw` role that ACTIVATES RLS enforcement. `db/migrations/0004_app_role.sql` creates `app_rw` (NOLOGIN/NOSUPERUSER/NOBYPASSRLS, no password in git) + grants least privilege (schema USAGE + CRUD on app tables + default privileges). The CI `db` job now runs ingest + retrieval AS `app_rw` (the real production path, a non-superuser subject to RLS) and asserts it is not superuser/bypassrls, sees 0 rows with no tenant GUC (fail-closed) and the demo tenant's 5 teams with it. NO app code change (the app already sets the GUC + reads `DATABASE_URL`). Policy intentionally NOT scoped `TO app_rw` (the PUBLIC policy already denies anon; `TO`-scoping would lock the owner out). Independent review = Ready to merge (0 Critical/Important, 1 doc minor applied); CI fully green. Operator runbook: docs/db/app-role.md. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-app-role*.
- OPERATOR STEP OPEN (slice 7 live activation): on the live Supabase, `alter role app_rw with login password '<secret>'`, then repoint the app's `DATABASE_URL` at `app_rw` via the SESSION pooler (docs/db/app-role.md). Until then the live app still connects as `postgres` (RLS enforcement-ready but not yet active live).
- DONE (session 22): real Supabase Auth wiring. `SupabaseAuthenticator` (in `sprintsight/web/auth/users.py`) now verifies passwords against Supabase Auth (GoTrue password grant, anon key only) behind a fail-safe gate (`make_authenticator()` -> `SeedAuthenticator` by default; Supabase only when `SPRINTSIGHT_AUTH=supabase` + `SUPABASE_URL` + `SUPABASE_ANON_KEY`). Session model unchanged (verifies creds + returns User; app keeps its signed cookie). SECURITY: role read ONLY from `app_metadata` (admin-set), never user-editable `user_metadata`; fails closed. No new dep (direct httpx). Eval-first `tests/web/auth/test_supabase_auth.py` (17, fully offline, httpx faked) incl. the self-set-role-ignored + network-branch cases; conftest forces `SPRINTSIGHT_AUTH=seed` for suite hermeticity. Review = Ready to merge (0 Critical/Important, 2 minors applied). 313 passed + 4 skipped, ruff clean. Operator runbook: docs/auth/supabase-auth.md. Spec/plan: docs/superpowers/{specs,plans}/2026-06-27-supabase-auth*.
- OPERATOR STEP OPEN (Supabase Auth live): create users in Supabase, set each role in `app_metadata` (NOT user_metadata), set `SPRINTSIGHT_AUTH=supabase` in the app env, verify a login (docs/auth/supabase-auth.md). Until then login uses the offline seed users.
- NEXT (real-wiring follow-ons): load the remaining delivery-domain rows (sprint/metric/burndown/dependency/signal) -> then the portfolio verdict/report could also run off DB rows (rest of Approach B); admin user-listing via the Supabase admin API; CSRF on `GET /logout` once it becomes a POST; OAuth/SSO + signup/reset flows.
- LATER: real Supabase Auth wiring (`SupabaseAuthenticator` stub) — needs a live Supabase project (operator step); CSRF on `GET /logout` once it becomes a POST; load the remaining delivery-domain rows (sprint/metric/burndown/dependency/signal) -> then the portfolio verdict/report could also run off DB rows (the rest of Approach B).
