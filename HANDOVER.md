...
- NEXT (real-wiring follow-ons): real Supabase Auth wiring (`SupabaseAuthenticator` stub, live project); load the remaining delivery-domain rows (sprint/metric/burndown/dependency/signal) -> then the portfolio verdict/report could also run off DB rows (rest of Approach B); CSRF on `GET /logout` once it becomes a POST.
